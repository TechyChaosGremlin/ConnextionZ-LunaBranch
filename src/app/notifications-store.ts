...
export async function fetchNotifications(): Promise<Result<AppNotification[]>> {
  ...
  items = result.value.map((notification) => ({
    ...
    actor: notification.actorId ?? undefined,
...
/** Maps the backend's snake_case notification type enum to the frontend's shape. */
function mapBackendNotificationType(type: string): NotificationType {
  switch (type) {
    case "new_like":
      return "like";
    case "new_comment":
