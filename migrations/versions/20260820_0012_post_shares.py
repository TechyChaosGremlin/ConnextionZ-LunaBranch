...
def upgrade() -> None:
    ...
    if "post_shares" in inspector.get_table_names():
        return
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("post_id", "user_id", name="uq_post_shares_pair"),
    )
    op.create_index(op.f("ix_post_shares_id"), "post_shares", ["id"], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index(op.f("ix_post_shares_id"), table_name="post_shares")
